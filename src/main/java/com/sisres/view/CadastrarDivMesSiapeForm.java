...
public class CadastrarDivMesSiapeForm extends ActionForm {
...
//		context.log("Método validate(CadastrarDivMesSIAPE) chamado!");
		...
		if (processoDataStr == null || processoDataStr.length() == 0 || processoDataStr.trim().equals("")) {
			errors.add("processoDataStr", new ActionMessage("error.campoprocessoStr.necessario"));
		} else {// (processoDataStr.length()!=0) {
		...
		if (banco == null || banco.trim().equals("")) {
		...
		if (contaCorrente == null || contaCorrente.trim().equals("")) {
